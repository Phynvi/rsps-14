...
public final class NpcDefinition {
	...
	public Model model() {
		...
		for (int index = 0; index < aditionalModels.length; index++)
			if (!Model.method463(aditionalModels[index]))
				flag1 = true;
		...
		for (int index = 0; index < aditionalModels.length; index++)
			models[index] = Model.method462(aditionalModels[index]);
		...
		if (recolourOriginal != null) {
			for (int index = 0; index < recolourOriginal.length; index++)
				model.method476(recolourOriginal[index], recolourTarget[index]);
	...
	public Model method164(int j, int somethingCurrentAnimsFrameNumber, int ai[], int nextFrame, int idk,
		...
		if (model == null) {
			...
			for (int j1 = 0; j1 < modelId.length; j1++)
				models[j1] = Model.method462(modelId[j1]);
			...
			model.method479(84 + lightModifier, 1000 + shadowModifier, -90, -580,
					-90, true);
			modelCache.put(model, interfaceType);
		}
		Model empty = Model.aModel_1621;
		...
		if (scaleXZ != 128 || scaleY != 128)
			empty.method478(scaleXZ, scaleXZ, scaleY);
		empty.method466();
	...
	public Model getAnimatedModel(int primaryFrame, int secondaryFrame,
		...
		if (model == null) {
			...
			for (int index = 0; index < modelId.length; index++)
				if (!Model.method463(modelId[index]))
					flag = true;
			if (flag) {
				return null;
			}
			Model models[] = new Model[modelId.length];
			for (int index = 0; index < modelId.length; index++)
				models[index] = Model.method462(modelId[index]);
			...
			if (recolourOriginal != null) {
				for (int index = 0; index < recolourOriginal.length; index++)
					model.method476(recolourOriginal[index],
							recolourTarget[index]);

			}
			model.method469();
			model.method479(64 + lightModifier, 850 + shadowModifier, -30, -50,
					-30, true);
			modelCache.put(model, interfaceType);
		}
