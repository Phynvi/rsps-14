...
public final class ObjectDefinition {
	...
	public Model modelAt(int type, int orientation, int aY, int bY, int cY, int dY, int frameId) {
		...
		if (contouredGround) {
			int y = (aY + bY + cY + dY) / 4;
			for (int vertex = 0; vertex < model.anInt1626; vertex++) {
				int x = model.anIntArray1627[vertex];
				int z = model.anIntArray1629[vertex];
				int l2 = aY + ((bY - aY) * (x + 64)) / 128;
				int i3 = dY + ((cY - dY) * (x + 64)) / 128;
				int j3 = l2 + ((i3 - l2) * (z + 64)) / 128;
				model.anIntArray1628[vertex] += j3 - y;
			}
	...
	/*	if (modifiedTexture != null) {			
		...
		model_3.method479(64 + ambientLighting, 1300 + (lightDiffusion * 5), -90, -280, -70, !delayShading);
		if (supportItems == 1)
			model_3.anInt1654 = model_3.modelBaseY;
