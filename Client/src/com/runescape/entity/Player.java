...
public final class Player extends Mob {
	...
	public Model getRotatedModel() {
		...
		if (super.graphic != -1 && super.currentAnimation != -1) {
			...
			if (spotAnimationModel  != null) {
				...
				model_3.method479(64 + spotAnim.modelShadow, 850 + spotAnim.modelBrightness, -30, -50, -30, true, true);
		...
		if (playerModel != null) {			
			...
			if (Client.tick >= objectModelStart && Client.tick < objectModelStop) {
				...
				model_1.method475(objectXPos - super.x, objectCenterHeight - anInt1709, objectYPos - super.y);
				...
				model_1.method475(super.x - objectXPos, anInt1709 - objectCenterHeight, super.y - objectYPos);
			}
		}
		animatedModel.aBoolean1659 = true;
		return animatedModel;
	...
	public Model getAnimatedModel() {
		...
		if (model_1 == null) {
			...
			for (int j3 = 0; j3 < 5; j3++)
				if (appearanceColors[j3] != 0) {
					model_1.method476(Client.PLAYER_BODY_RECOLOURS[j3][0],
							Client.PLAYER_BODY_RECOLOURS[j3][appearanceColors[j3]]);
					if (j3 == 1)
						model_1.method476(Client.anIntArray1204[0], Client.anIntArray1204[appearanceColors[j3]]);
				}
		...
		Model emptyModel = Model.aModel_1621;		
		...
		emptyModel.method466();
		emptyModel.anIntArrayArray1658 = null;
		emptyModel.anIntArrayArray1657 = null;
		return emptyModel;
	}
	...
	public Model getHeadModel() {
		...
		for (int index  = 0; index  < 5; index ++) {			
			if (appearanceColors[index ] != 0) {
				headModel.method476(Client.PLAYER_BODY_RECOLOURS[index ][0],
						Client.PLAYER_BODY_RECOLOURS[index ][appearanceColors[index ]]);
				if (index  == 1) {
					headModel.method476(Client.anIntArray1204[0], Client.anIntArray1204[appearanceColors[index ]]);
				}
			}
		}
