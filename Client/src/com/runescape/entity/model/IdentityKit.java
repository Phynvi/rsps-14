...
public final class IdentityKit {
	...
	public boolean bodyLoaded() {
		...
		for (int part = 0; part < bodyModels.length; part++)
			if (!Model.method463(bodyModels[part]))
				ready = false;
	...
	public Model bodyModel() {
		...
		for (int part = 0; part < bodyModels.length; part++)
			models[part] = Model.method462(bodyModels[part]);
		...
		for (int part = 0; part < 6; part++) {
			if (originalColors[part] == 0)
				break;
			model.method476(originalColors[part], replacementColors[part]);
		}
	...
	public boolean headLoaded() {
		...
		for (int part = 0; part < 5; part++)
			if (headModels[part] != -1 && !Model.method463(headModels[part]))
				ready = false;
	...
	public Model headModel() {
		...
		for (int part = 0; part < 5; part++)
			if (headModels[part] != -1)
				models[count++] = Model.method462(headModels[part]);
